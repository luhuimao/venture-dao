...
describe("funding...", () => {
    ...
    it("priority deposit...", async () => {
        ...
        const freeInEscrowAmount1 = await this.flexFreeInEscrowFundAdapterContract.getEscrowAmount(dao.address, proposalId, this.owner.address);
        const freeInEscrowAmount2 = await this.flexFreeInEscrowFundAdapterContract.getEscrowAmount(dao.address, proposalId, this.investor1.address);

        const priorBal1 = await flexFundingPoolExtContract.getPriorAmount(proposalId, this.owner.address, flexFundingProposalInfo.executeBlockNum - 1);
        const priorBal2 = await flexFundingPoolExtContract.getPriorAmount(proposalId, this.investor1.address, flexFundingProposalInfo.executeBlockNum - 1);

        const pdamount = await flexFundingPoolAdapt.freeINPriorityDeposits(dao.address, proposalId);
        console.log(`
            processed...
            deposit balance   ${hre.ethers.utils.formatEther(depositeBal.toString())}
            deposit balance2   ${hre.ethers.utils.formatEther(depositeBal2.toString())}

            freeInEscrowAmount1   ${hre.ethers.utils.formatEther(freeInEscrowAmount1[1].toString())}
            freeInEscrowAmount2   ${hre.ethers.utils.formatEther(freeInEscrowAmount2[1].toString())}

            priorBal1   ${hre.ethers.utils.formatEther(priorBal1.toString())}
            priorBal2   ${hre.ethers.utils.formatEther(priorBal2.toString())}

            freeINPriorityDeposits   ${hre.ethers.utils.formatEther(pdamount.toString())}
        ...
        console.log(`
            claimed...
            claimed: ${hre.ethers.utils.formatEther(createdVestingInfo.claimed)}
            total1: ${hre.ethers.utils.formatEther(createdVestingInfo.total)}
